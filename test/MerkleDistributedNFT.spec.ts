...
import Distributor from '../build/MerkleDistributedNFT.json'
...
describe('MerkleDistributedNFT', () => {
  ...
  describe('#merkleRoot', () => {
    it('returns the zero merkle root', async () => {
      const distributor = await deployContract(wallet0, Distributor, ["Merkle NFT", "MERK", ZERO_BYTES32], overrides)
  ...
  describe('#claim', () => {
    ...
    describe('two account tree', () => {
      ...
      beforeEach('deploy', async () => {
        ...
        distributor = await deployContract(wallet0, Distributor, ["Merkle NFT", "MERK", tree.getHexRoot()], overrides)
      })
      ...
      it('transfers the token', async () => {
        const proof0 = tree.getProof(0, wallet0.address, BigNumber.from(100))
        expect(await distributor.balanceOf(wallet0.address)).to.eq(0)
        await distributor.claim(0, wallet0.address, 100, proof0, overrides)
        expect(await distributor.balanceOf(wallet0.address)).to.eq(1)
      })
      ...
      it('gas', async () => {
        ...
        expect(receipt.gasUsed).to.eq(198082)
      })
    })
  ...
  describe('parseBalanceMap', () => {
    ...
    beforeEach('deploy', async () => {
      ...
      claims = innerClaims
      distributor = await deployContract(wallet0, Distributor, ["Merkle NFT", "MERK", merkleRoot], overrides)
    })
